...
let readDict (preferredArr: string list) =
    ...
    // reading in files
    let dict = File.ReadAllText "new_cmu_pos_dict.txt"
    ...
    let dummy = dictArr |> Array.map (fun i -> 
        ...
        // we consider the rhyme the last 2 syllables matching unless there is only 1
...
*)
let convert (lines: TranslationUnit list) (wtf: Hashtable) (ftw: Hashtable) (cftw: Hashtable) (pftw: Hashtable): string list=
        ...
        | x::xs ->
                ...
                if x.translate = true then
                    if reuse.ContainsKey(x.word.ToLower()) then 
                        ...
                        if VERBOSE then printfn "%A, %A, %A, %A" x.word fixedWord correctFeat tid
...
let evalProg (ps: Expr list) = 
    ...
    let rec matchAbstractType (ls: Expr list) = 
        ...
        | x::xs ->
            ...
            | Line x ->
                (convert x wtf ftw cftw pftw) :: matchAbstractType xs
